...
The main adaptation is to split the retrospective in two. The divergent part, writing notes under each L, happens asynchronously over a day or two, so everyone can contribute at a time that suits them. The convergent part, discussing themes and agreeing on actions, happens in a shorter live video session. [GitLab's handbook](https://handbook.gitlab.com/handbook/engineering/management/group-retrospectives/) recommends exactly this for its own groups: collect feedback asynchronously in an issue so people can think on their own time, and get everyone face to face on video after a difficult iteration or when emotions are likely to run high.
