...
| Time to Learn | 45-90 minutes per objective |
...
| Prerequisites | A drafted objective (qualitative goal statement) to attach key results to, Access to current baseline metrics or the ability to establish them, Basic understanding of the OKR framework structure (objectives vs. key results), Familiarity with your team's or product's core metrics and how they are tracked |
...
The reason the baseline matters so much is that it anchors ambition in reality. A target of 45% retention sounds aggressive if your baseline is 42% and modest if your baseline is 20%. Without the baseline, you cannot set an appropriate stretch, and you cannot score the key result on the 0.0-1.0 scale that the [OKR framework](https://tryhamster.com/methods/objectives-and-key-results-okrs) uses at end of cycle. Gathering the baseline is often the most time-consuming part of writing key results, but it is also the most valuable, because it forces you to confirm that the metric actually exists and is being tracked before you commit to moving it.
...
### Step 1: Step 1: Restate the Objective and Identify What Success Looks Like
...
> **Pro tip:** Involve 2-3 people who are close to the work when brainstorming signals. Solo brainstorming tends to anchor on the one or two metrics you already watch, missing dimensions that someone in a different role would catch immediately.

### Step 2: Step 2: Filter Candidates for Measurability and Data Availability
...
> **Pro tip:** A surprisingly common failure mode is picking a metric you can track but cannot influence within the cycle. Before keeping a candidate, ask: "Do we have a plausible theory for how our planned work will move this number?" If not, it is a monitoring metric, not a key result.

### Step 3: Step 3: Establish Baselines for Each Surviving Candidate
...
> **Pro tip:** Resist the urge to round baselines. If your activation rate is 28.3%, write 28.3%, not "about 28%." Rounding introduces ambiguity at scoring time and makes it harder to detect small but real improvements.

### Step 4: Step 4: Select 2-5 Key Results That Cover Different Dimensions
...
> **Pro tip:** Three key results per objective is the sweet spot for most teams. Two works for narrow objectives. Five is the upper bound, and teams that use five often find that the fourth and fifth get neglected during check-ins.

### Step 5: Step 5: Set Ambitious but Achievable Targets
...
> **Pro tip:** When setting targets for metrics you have never tracked before (baseline is zero), talk to peers at similar companies or use industry benchmarks to sanity-check your target. Without a reference point, teams either sandbag or set impossible numbers.

### Step 6: Step 6: Write Each Key Result in Standard Format
...
> **Pro tip:** Keep a shared glossary of metric definitions. "Activation rate" might mean different things to product, marketing, and sales. Pin down the exact definition (e.g., "percentage of new signups in a cohort who complete onboarding and perform one core action within 7 days") and link to it from the key result.

### Step 7: Step 7: Assign a Data Source and Measurement Cadence to Each Key Result
...
> **Pro tip:** Run a "day one drill" before the quarter starts: actually pull each metric from the stated data source. If the pull takes more than 5 minutes or requires special access, fix the tooling or simplify the metric. If check-in measurement is painful, it will not happen consistently.

### Step 8: Step 8: Pressure-Test with Stakeholders
...
> **Pro tip:** Time-box the review to 30 minutes. If the conversation drags beyond that, it usually means the objective itself is unclear, not just the key results. Escalate to an objective rewrite rather than endlessly wordsmithing key results.

### Step 9: Step 9: Finalize and Lock Key Results for the Cycle
...
Send the finalized OKRs to all stakeholders who reviewed them, confirming that the feedback loop is closed. This artifact is now the reference document for [check-ins](https://tryhamster.com/skills/conducting-okr-check-ins-and-reviews) and [end-of-cycle scoring](https://tryhamster.com/skills/scoring-and-grading-okrs).

> **Pro tip:** Store finalized key results in a location that is not editable by the team without a visible change log. This prevents quiet mid-cycle target adjustments that undermine accountability. If a key result genuinely needs to change, do it explicitly in a check-in and document the reason.
...
- **Writing tasks or milestones instead of measurable outcomes** — This is the single most common key result mistake. " These are outputs, not outcomes. The team can ship a redesigned checkout flow that nobody uses and still check the box. " Then make that metric the key result.
...
- **Using binary key results for gradual changes** — 0, which means every check-in before completion shows zero progress. This makes mid-quarter reviews uninformative and demoralizing. It also creates a perverse incentive to rush completion just before the deadline. Fix it by decomposing the binary outcome into measurable milestones or percentages.
