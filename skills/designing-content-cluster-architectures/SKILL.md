...
Every supporting page links to the pillar, and the pillar links to every supporting page. HubSpot's report says every post in a cluster should be linked to at least once with the same anchor text so a search engine sees it as part of the cluster. Google's [link best practices](https://developers.google.com/search/docs/crawling-indexing/links-crawlable) ask for anchor text that is "descriptive, reasonably concise, and relevant". Consistent, descriptive anchors satisfy both.
