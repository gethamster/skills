...
1. Define a registry with `setup({ use: { /* actors */ } })`.
2. Expose `registry.serve()` or `registry.handler()` (serverless) or `registry.startRunner()` (runner mode). Prefer serverless mode unless the user has a specific reason to use runner mode.
3. Verify `/api/rivet/metadata` returns 200 before deploying.
4. Configure Rivet Cloud or self-hosted engine
...
1. Integrate clients (see client guides below for JavaScript, React, or Swift)
2. Prompt the user if they want to deploy. If so, go to Deploying Rivet Backends.
