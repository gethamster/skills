...
{ "scripts": { "build": "next build", "lint": "oxlint", "test": "vitest" } }

// apps/ai/package.json
{ "scripts": { "build": "tsc", "lint": "oxlint", "test": "vitest" } }
...
├─ Lint configuration → references/best-practices/structure.md#lint-configuration
├─ Dependency management → references/best-practices/dependencies.md
...
1. **If dependencies are NOT declared**, the `prebuild` exists because `^build` won't trigger without a dependency relationship. The fix is to:
...
| [best-practices/structure.md](./references/best-practices/structure.md) | Repository structure, workspace config, TypeScript and Oxlint setup |
