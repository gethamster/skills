...
├── utils/
├── config/
│   ├── oxlint/            # Optional: shared rule presets
│   ├── typescript/
│   └── tailwind/
└── features/
    ├── auth/
...
Prefer a **repository root** `.oxlintrc.json` and root `package.json` scripts (`oxlint`, `oxlint --fix`). Formatting uses **Oxfmt** (`.oxfmtrc.jsonc`).
...
├── .oxlintrc.json
├── .oxfmtrc.jsonc
└── package.json   # "lint": "oxlint", "format": "oxfmt --check"
```

Optional shared presets can live under `packages/config/*`; many monorepos only need the root JSON files.
