...
Monitoring is **not** wired as a `@Catch` filter: explicit `MonitoringService.captureException` calls from services keep the responsibility where domain context lives.

### Layer 1: Supabase Exception Filter (Database/Auth Errors)
...
### Layer 2: Zod Validation Exception Filter (Input Validation)
...
2. **Capture from the service layer, not from the filter** — services own the domain context and call `MonitoringService.captureException` explicitly; filters own response shape
...
### Pattern: PostHog Error Tracking Integration

For production monitoring, route exceptions explicitly through `@kit/monitoring/posthog`:

```typescript
// Inject MonitoringService where you have domain context (services, processors).
// Filters handle response shape; they should not own the capture call.
constructor(private readonly monitoring: MonitoringService) {}

try {
  await this.work();
} catch (error) {
  this.logger.error({ msg: 'Work failed', ...toErrorContext(error) });
  this.monitoring.captureException(error, { layer: 'service', /* domain ids */ });
  throw error;
}
```
