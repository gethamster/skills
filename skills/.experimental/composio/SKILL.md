...
Check if the CLI is installed; if not, download the installer, review it, then run it (avoid piping remote scripts straight to bash):

```bash
curl -fsSL https://composio.dev/install -o /tmp/composio-install.sh
# Inspect /tmp/composio-install.sh before executing
...
For agents without direct browser access: `composio login --no-wait | jq` to get a device-auth URL and a session CLI key. Give the URL to the account owner you are acting for (it only lets them complete their own OAuth login in their browser), then run `composio login --key <cli_key> --no-wait` once they finish. Treat the CLI key as a session credential: never share it with anyone else, post it in chat/issues, or commit it.
