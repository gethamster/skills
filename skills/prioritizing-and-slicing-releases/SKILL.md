...
When a necessary task will not fit, there are standard moves. Patton suggests asking, for each user, whether they can do their work with the slice, whether a paper process or software workaround covers an omitted feature, and whether a feature "could be split into a crude minimal version for earlier release and a more elaborate version for later release." The Nielsen Norman Group adds that risky cards, such as ones not backed by user data or not technically feasible, can be swapped for leaner alternatives with the same value so the team learns before investing ([NN/g](https://www.nngroup.com/articles/user-story-mapping/)).
