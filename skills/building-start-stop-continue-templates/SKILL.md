...
| Prerequisites | Basic understanding of the Start Stop Continue framework, Familiarity with at least one digital collaboration tool (Miro, Google Docs, or Notion) |
...
Once you've built your template library, you can pair it with skills like [facilitating Start Stop Continue retrospectives](https://tryhamster.com/skills/facilitating-start-stop-continue-retrospectives) and [crafting actionable feedback prompts](https://tryhamster.com/skills/crafting-actionable-feedback-prompts) to run sessions that consistently produce real improvements.
...
### Step 1: Step 1: Choose Your Medium Based on Team Context
...
> **Pro tip:** Don't introduce a new tool just for retrospectives. The best template is one that lives where your team already works—adoption beats aesthetics every time.

### Step 2: Step 2: Design the Three-Column Core Structure
...
> **Pro tip:** In Miro, use frames to create the three sections and lock the structural elements so participants can only add sticky notes, not accidentally move the template.

### Step 3: Step 3: Add Guiding Prompts to Each Section
...
You can explore prompt design in depth with the sibling skill on [crafting actionable Start Stop Continue questions and prompts](https://tryhamster.com/skills/crafting-actionable-feedback-prompts). The key here is to bake 2-3 prompts directly into the template so they're visible when participants start writing.

> **Pro tip:** Rotate prompts every 3-4 sessions to prevent "prompt fatigue" where participants start ignoring them. Keep a prompt library in a separate document.

### Step 4: Step 4: Build in Prioritization and Voting Mechanisms
...
> **Pro tip:** Cap votes at 3 per person for teams under 8 people and 5 for larger groups. Too many votes dilutes the signal.

### Step 5: Step 5: Include an Action Items Section
...
> **Pro tip:** Pre-fill the action items section with any unresolved items from the previous session. This creates accountability and continuity between retrospectives.

### Step 6: Step 6: Add Session Metadata and Instructions
...
> **Pro tip:** Write instructions assuming someone is using the template for the first time. Your future self—or a new team member—will thank you.

### Step 7: Step 7: Test, Iterate, and Templatize
...
> **Pro tip:** Create a "template changelog" note that records what you modified after each session. Over 3-4 iterations, you'll converge on a design that fits your team perfectly.
...
- **Creating a beautiful template with no action items section, treating the session as a venting exercise rather than a decision-making process.** — Always include a structured action items section with fields for owner, deadline, and status. If your template doesn't produce commitments, it's a survey, not a retrospective tool.
- **Making the template too complex with dozens of fields, sub-categories, and rating scales that overwhelm participants before they write a single item.** — Start minimal: three sections, 2-3 prompts each, a voting mechanism, and action items. Add complexity only after running 2-3 sessions and identifying specific gaps.
- **Using the same generic prompts for every session regardless of context, leading to stale, repetitive feedback after the third or fourth use.** — Customize prompts to the specific sprint, project phase, or team challenge. Maintain a rotating prompt library and swap in fresh questions every few sessions. See the sibling skill on crafting actionable feedback prompts for guidance.
- **Building a synchronous-only Miro board for a distributed team across multiple time zones, resulting in low participation from anyone who can't attend the live session.** — For distributed teams, design an asynchronous-first template (Google Form → Sheet, or Notion database) with a defined submission window, then use a shorter synchronous session only for discussion and prioritization.
- **Not locking the template structure in collaborative tools, allowing participants to accidentally move, resize, or delete frames and columns.** — In Miro, lock all structural elements and only allow sticky note creation. In Google Docs, use protected ranges for headers and instructions. In Notion, use locked database properties.
